({
	 draupAuthenticationHelper : function(component, event, helper) {
         ...
         var draupApiUrl = $A.get("$Label.c.Draup_Platform_Url");
         var queryParameters;
         var msg = 'Client Id or Client Secret is missing. Please Contact Administrator to provide client details in Admin Page';
         var isSuperUser = component.get('v.isSuperUser');
         if(isSuperUser == true )
         {
             msg ='Client Id or Client Secret is missing. Please provide client details in Admin page and try again';
         }
         ...
         action.setCallback(this, function(response) {
             ...
             if (state === "SUCCESS") {
                 ...
                 if(queryParameters != null)
                 {
                     ...
                     var loginWindow = window.open(draupApiUrl+"/draup/authorize?client_id="+queryParameters,"_blank",'height=600,width=600,location=yes,left=400,top=100,location=center,Window Position=center,toolbar=no,status=no,menubar=no,scrollbars=1', 1)       
    ...
    getRoledexSuggestions : function(component, event, helper) {
        ...
        var msg = 'Record hardly linked with Daup, Please contcat DraupSupport team for futher action.';
        var toastEventforAutoLink = $A.get("e.force:showToast");
        toastEventforAutoLink.setParams({
            duration:1000,
            "title": "Account Linked with Draup",
            "type": 'Warning',
            "message": msg
        });
        ...
        action.setCallback(this, function(a) {
            if (a.getState() === "SUCCESS") {
                ...
                var isFromunlink = component.get('v.isFromUnlink');
                var isSuperUser = component.get('v.isSuperUser');
              ...
              if(!isSuperUser){
                 ...
                 else if(accounts.length==1 && accounts[0].direct_match == true){
                    console.log('from length ==1')
                    component.set("v.accId",accounts[0].account_id);
                    component.set("v.draupAccountName",accounts[0].account_name);
                    //this.showSpinner(component, true);
                    console.log(' isFromunlink '+ isFromunlink);
                    if(isFromunlink == true )
                    {
                        component.set('v.showPopup',true);
                        component.set("v.isFromUnlink",false);
                    }
    ...
    currentUserInfo : function(component, event, helper) {
        ...
        component.set('v.noDataFlag','slds-hide');
        //this.showSpinner(component, true);
        ...
        action.setCallback(this, function(response) {
            ...
            var expiryDate = component.get("v.userInfo").draupApp__ExpiryDate__c;
			 if((userToken == null || expiryDate == null) || (userToken != null && expiryDate <= new Date().toISOString().slice(0,10) ))
            {
        ...
        actionToDelete.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                if(bResponse === true)
                {
                    ...
                    component.set('v.isFromUnlink',true);
